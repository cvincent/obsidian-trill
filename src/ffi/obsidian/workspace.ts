...
export function get_leaves_of_type(
  workspace: Workspace,
  view_type: string,
): List<WorkspaceLeaf> {
...
export async function leaf_set_view_state(
  leaf: WorkspaceLeaf,
  view_type: string,
  active: boolean,
) {
...
export function open_link_text(
  workspace: Workspace,
  path: string,
  pane_type: PaneType,
): void {
...
export function on_layout_ready(
  workspace: Workspace,
  callback: () => void,
): void {
